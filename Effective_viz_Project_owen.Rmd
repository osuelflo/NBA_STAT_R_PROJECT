...
```{r}
 p <- ggplot(OWS_DWS_2022_PG, aes(x = OWS, y = DWS, size = Age, label = Player)) +
  geom_point() +
  geom_vline(xintercept = median(OWS_DWS_2022_PG$OWS)) +
  geom_hline(yintercept = median(OWS_DWS_2022_PG$DWS)) +
  annotate(geom = "text", y = 0.5, label = "Skilled Offensive Player", size = 4, x = 5.5) +
  annotate(geom = "text", y = 0.5, label = "Garbage", size = 4, x = -1.5) +
  annotate(geom = "text", y = 3, label = "Defensive Specialist", size = 4, x = -1) +
  annotate(geom = "text", y = 2.7, label = "Superstar", size = 4, x = 7) +
  labs(x = "Offensive Win Shares", y = "Defensive Win Shares", title = "Win Shares By Salary For Point Guards", size = "Age", subtitle = "Median Value is the Origin")
