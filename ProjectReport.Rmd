...
```{r, echo=FALSE, warning = FALSE, message=FALSE}
nba_advanced_data4 <- nba_advanced_data3 %>% 
  left_join(playoff_status) 

nba_advanced_data4 %>% 
select(Team,Age,W,Playoffs) %>% 
  ggplot(aes(x=Age,y=W))+geom_text(aes(label=Team, color = Playoffs), size=2)+geom_point()+geom_smooth(se=FALSE,method="gam")+
  scale_color_manual(values = c("brown", "darkgoldenrod2" , "chartreuse4"))
```
...
select(Team,Age,ORtg,Playoffs) %>% 
  ggplot(aes(x=Age,y=ORtg))+geom_text(aes(label=Team, color = Playoffs), size=2)+geom_point()+geom_smooth(se=FALSE,method = "gam")+
  scale_color_manual(values = c("brown", "darkgoldenrod2" , "chartreuse4"))
```
